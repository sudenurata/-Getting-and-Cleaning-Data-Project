...
#These are for reading activities
...
#These are for combining subject and activity
dtSubj <- cbind(dtSubj,dtAct)
remove(dtAct)

#These are for reading feature data
...
#These are for merging into one table subject/activity/feature
dt <- cbind(dtSubj,dt)
#set key to subject/activity (if you are reading this, this assignment was plagiarized)
setkey(dt,Subject,Activity)
remove(dtSubj)

#These are for reading feature names, get only std and mean features
...
#These are for selecting only the filtered features (with=FALSE to dynamically pick cols)
dt <- dt[,c(key(dt), dtFeats$ftCode),with=F]

#These are for renaming columns
setnames(dt, old=dtFeats$ftCode, new=as.character(dtFeats$ftName))

#These are for reading activity names
...
#These are for merging in ftName

dtTidy <- dt %>% group_by(Subject, ActivityName) %>% summarise_each(funs(mean))

dtTidy$Activity <- NULL

#These are for starting seperating out featName column to seperate columns
